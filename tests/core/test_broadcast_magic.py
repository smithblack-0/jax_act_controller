...
from src.jax_act.core.broadcast_magic import BroadcastEditing
...
class test_helper_functions(unittest.TestCase):
    def test_execute_binary_operator_simple(self):
        ...
        result = BroadcastEditing.execute_binary_operator(operator, operand_a, operand_b, context_msg)
        result = BroadcastEditing.execute_binary_operator(operator, operand_b, operand_a, context_msg)
    ...
    def test_execute_binary_operator_jit(self):
        ...
        operand_b = jnp.ones([3, 5])
        expected_result = operand_a[..., None]-operand_b
        func = jax.jit(BroadcastEditing.execute_binary_operator, static_argnums=[3])
    ...
    def test_execute_binary_operator_raises_utility_wrapper(self):
        ...
        with self.assertRaises(RuntimeError) as err:
            BroadcastEditing.execute_binary_operator(jnp.add, tree_one, tree_two, context_msg)
    ...
    def test_execute_binary_operator_raises_bad_operator(self):
        ...
        with self.assertRaises(RuntimeError) as err:
            BroadcastEditing.execute_binary_operator(lambda x : x, operand_one, operand_two,
                                                     context_message)
        if SHOW_ERROR_MESSAGES:
            print(err.exception)
            print(err.exception.__cause__)
    ...
    def test_execute_unitary_operator_with_pytree(self):
        ...
        expected_output = {"item1" : -jnp.ones([3, 5]),
                           "item2" : -jnp.ones([3,7])}
        result = BroadcastEditing.execute_unitary_operation(operator, operand, context_msg)
    ...
    def test_execute_unitary_operator_raise_bad_operator(self):
        ...
        operand = jnp.ones([3, 2])
        with self.assertRaises(RuntimeError) as err:
            BroadcastEditing.execute_unitary_operation(operator, operand, context_msg)
        if SHOW_ERROR_MESSAGES:
            print(err.exception)
            print(err.exception.__cause__)
