...
class test_execute_operators(unittest.TestCase):
    def test_execute_binary_operator_simple(self):
        ...
        result = utils.execute_binary_operator(operator, operand_a, operand_b, context_msg)
        result = utils.execute_binary_operator(operator, operand_b, operand_a, context_msg)
    ...
    def test_execute_binary_operator_jit(self):
        ...
        operand_b = jnp.ones([3, 5])
        expected_result = operand_a[..., None]-operand_b
        func = jax.jit(utils.execute_binary_operator, static_argnums=[3])
    ...
    def test_execute_binary_operator_raises_utility_wrapper(self):
        ...
        with self.assertRaises(RuntimeError) as err:
            utils.execute_binary_operator(jnp.add, tree_one, tree_two, context_msg)
        if SHOW_ERROR_MESSAGES:
            print(err.exception)
    ...
    def test_execute_binary_operator_raises_bad_operator(self):
        ...
        with self.assertRaises(RuntimeError) as err:
            utils.execute_binary_operator(lambda x : x, operand_one, operand_two,
                                                    context_message)
        if SHOW_ERROR_MESSAGES:
            print(err.exception)
            print(err.exception.__cause__)
